...
namespace OpcUaTest
{
    ...
    {
        protected readonly OpcUaSession Client;
        protected readonly Random Random;
        protected readonly Action<string> Output;

        protected OpcUaTestRunner(OpcUaSession client, Action<string> output)
        {
            Client = client;
            Output = output;
        ...
        protected void Run<T>(T value, string node, Func<T, T, bool> compare, Func<T, string> output=null)
        {
            ...
            Output?.Invoke($"Write> {node}={(output == null ? value.ToString() : output(value))}:{ret}");
            var read = Client.Session.Read<T>(node);
            Output?.Invoke($"Read> {node}={(output == null ? read.ToString() : output(read))}:{compare(read, value)}");
            Output?.Invoke(new string('-', 48));
        }
        ...
        protected void RunEncodeableObject<T>(T value, string node, Func<T, string> output, Func<T, T, bool> compare) where T : EncodeableObject
        {
            ...
            //var raw = Client.Session.Read(node);
            var val = Client.Session.Read<T>(node);
            Output?.Invoke($"Read> {node}={{{output(val)}}}:{compare(value, val)}");
            Output?.Invoke(new string('-', 48));
        }
